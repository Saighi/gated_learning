...
#nb total inputs = max(nb_tot_connections,nb_conso*nb_connection_input_conso)
nb_conso = 10
nb_stud_inputs = 50
nb_connection_input_conso = 2000
beta_student = 1 / 50
nb_students_per_conso = 50
nbtot_connections = nb_students_per_conso * nb_stud_inputs * nb_conso
inhib_post_val = 0.2  # Prev 0.3
inhib_post_val_conso = 0.1 / nb_students_per_conso
...
network_input = SpikeGeneratorGroup(max(nbtot_connections,nb_conso*nb_connection_input_conso), indices_inputs, times_inputs)
teacher = NeuronGroup(1, eqs_teacher, threshold='v>1', reset='v=0', method='exact')
student = NeuronGroup(nb_students_per_conso * nb_conso, eqs, threshold='v>1', reset='v = 0', method='exact',
...
consolidation_unit = NeuronGroup(nb_conso, eqs_conso, threshold='v>1', reset='v=0', method='euler',
...
j_connects = np.repeat(np.arange(0, nb_students_per_conso * nb_conso), nb_stud_inputs)
...
i_connects_conso = np.random.choice(np.arange(0,nbtot_connections), nb_connection_input_conso*nb_conso, replace=False)
j_connects_conso = np.repeat(np.arange(0, nb_conso), nb_connection_input_conso)
...
i_connects_stud_conso = np.random.choice(np.arange(0, nb_conso * nb_students_per_conso),
                                         nb_conso * nb_students_per_conso, replace=False)
j_connects_stud_conso = np.repeat(np.arange(0, nb_conso), nb_students_per_conso)
syn_students_conso.connect(i=i_connects_stud_conso, j=j_connects_stud_conso)
...
repertory = "data_conso_many_dcn"
"""Saving data :"""
for i in range(100):
    np.savetxt(repertory+"/syn_w_input_student_" + str(i) + ".data", syn_mon.w[i])
np.savetxt(repertory+"/syn_w_input_student_times.data", syn_mon.t)
for i in range(nb_connection_input_conso):
    np.savetxt(repertory+"/syn_w_input_conso_" + str(i) + ".data", syn_mon_conso.w[i])
np.savetxt(repertory+"/syn_w_input_conso_times.data", syn_mon_conso.t)
np.savetxt(repertory+"/syn_source_input_conso.data", i_connects_conso)

np.savetxt(repertory+"/spikes_teachers_index.data", teacher_mon.i)
np.savetxt(repertory+"/spikes_teachers_times.data", teacher_mon.t)
np.savetxt(repertory+"/spikes_student_index.data", student_mon.i)
np.savetxt(repertory+"/spikes_student_times.data", student_mon.t)
np.savetxt(repertory+"/spikes_to_learn_index.data", to_learn_mon.i)
np.savetxt(repertory+"/spikes_to_learn_times.data", to_learn_mon.t)
np.savetxt(repertory+"/sync_detector_voltage.data", sync_mon.v[0])
np.savetxt(repertory+"/sync_detector_spikes.data", sync_spike_mon.t)
np.savetxt(repertory+"/sync_detector_voltage_2.data", sync_mon_2.v[0])
np.savetxt(repertory+"/sync_detector_spikes_2.data", sync_spike_mon_2.t)
np.savetxt(repertory+"/conso_spikes.data", conso_spike_mon.t)
"""
plot(M.t / ms, M.v[0])
xlabel('Time (ms)')
ylabel('v')
